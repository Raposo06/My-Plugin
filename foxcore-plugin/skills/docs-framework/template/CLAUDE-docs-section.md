...
Paste this into the project's `CLAUDE.md`. **This is the load-bearing part of the
framework** — the file skeletons are just shape; this is the contract that makes
them get updated instead of rotting.

Adjust the table rows to the files you actually create. Delete rows you don't
have; a row pointing at a missing file is worse than no row.
